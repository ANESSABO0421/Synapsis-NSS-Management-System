...
import TeacherEventList from "./TeacherEventList";
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Assuming backend returns { success, data: [ { _id, name, date, description } ] }
        ...
        <AnimatePresence mode="wait">
          {selectedEvent ? (
            <motion.div
              ...
              className="h-full"
            >
              <TeacherMessagePanel event={selectedEvent} socket={socket} />
            </motion.div>
          ) : (
            <motion.div
              key="empty-state"
